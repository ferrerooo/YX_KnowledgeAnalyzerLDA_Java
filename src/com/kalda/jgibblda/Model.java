	...
	public boolean saveModelTwords(String filename){
		...
		try{
			...
			for (int k = 0; k < K; k++){
				...
				// for calculating total probability
				//Double totle = new Double(0);
				for (int i = 0; i < twords; i++){
					...
					if (data.localDict.contains((Integer)wordsProbsList.get(i).first)){
						...
						writer.write(" " + word + " " + StringUtil.She4ru5ReturnString(4, d) + "\n");
//						if (i<twords-1) {
//							Double d = (Double) wordsProbsList.get(i).second;
//							totle = totle + d;
//							writer.write(" " + word + " " + StringUtil.She4ru5(4, d) + "\n");
//						}
//						if (i==twords-1) {
//							Double d = (Double) wordsProbsList.get(i).second;
//							totle = totle + d;
//							writer.write(" " + word + " " + StringUtil.She4ru5(4, d) + " t-"+StringUtil.She4ru5(4, totle) + "\n");
//						}
					}
				}
			} //end foreach topic			
	...
	public boolean saveModel(String modelName){
		if (!saveModelTAssign("\\src\\models\\casestudy" + File.separator + modelName + tassignSuffix)){
			return false;
		}
		
		if (!saveModelOthers("\\src\\models\\casestudy" + File.separator + modelName + othersSuffix)){			
			return false;
		}
		
		if (!saveModelTheta("\\src\\models\\casestudy" + File.separator + modelName + thetaSuffix)){
			return false;
		}
		
		if (!saveModelPhi("\\src\\models\\casestudy" + File.separator + modelName + phiSuffix)){
			return false;
		}
		
		if (twords > 0){
			if (!saveModelTwords("\\src\\models\\casestudy" + File.separator + modelName + twordsSuffix))
				return false;
		}
		return true;
